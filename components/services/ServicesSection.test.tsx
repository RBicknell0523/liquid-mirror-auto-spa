...
describe("ServicesSection", () => {
  ...
  it("adds the selected service to the booking cart and closes the modal", async () => {
    renderServices()
    await userEvent.click(screen.getAllByRole("button", { name: /view details/i })[0])
    await userEvent.click(screen.getByRole("button", { name: /add to booking/i }))

    // Modal closes on add.
    expect(screen.queryByRole("dialog", { name: /liquid mirror maintenance wash/i })).not.toBeInTheDocument()
  })
})
